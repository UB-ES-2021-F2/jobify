...
export default {
  ...
  methods: {
    ...
    getWorkExperience () {
      const path = Vue.prototype.$API_BASE_URL + 'work_experience/' + this.companyname_profile
      axios.get(path)
        .then((res) => {
    ...
    getEducation () {
      const path = Vue.prototype.$API_BASE_URL + 'education/' + this.companyname_profile
      axios.get(path)
        .then((res) => {
    ...
    getName () {
      const pathJobseeker = Vue.prototype.$API_BASE_URL + 'jobseeker/' + this.companyname_profile.toLowerCase()
  ...
  created () {
    this.companyname_profile = this.$route.path.split('company/')[1].toLowerCase()
    ...
    this.edit_mode = this.username === this.companyname_profile
    this.getName()
    this.getWorkExperience()
    this.getEducation()
  }
